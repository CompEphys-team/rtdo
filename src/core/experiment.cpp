...
std::vector<scalar> Experiment::errProfile(const Stimulation &I, std::vector<scalar> model, size_t targetParam)
{
    ...
    std::vector<scalar> ret(lib.expd.numCandidates);

    // Set parameter values
    for ( size_t i = 0; i < lib.adjustableParams.size(); i++ ) {
        if ( i == targetParam ) {
            for ( size_t j = 0; j < lib.expd.numCandidates; j++ )
    ...
    // Reset err
    for ( size_t i = 0; i < lib.expd.numCandidates; i++ )
        lib.err[i] = 0.0;
    ...
    // Settle
    Stimulation settle {};
    settle.duration = lib.expd.settleDuration;
...
scalar Experiment::errProfile_value(size_t targetParam, size_t idx)
{
    ...
    return p.min + (p.max-p.min) * idx / (lib.expd.numCandidates-1);
}

size_t Experiment::errProfile_idx(size_t targetParam, scalar value)
{
    const AdjustableParam &p = lib.adjustableParams[targetParam];
    double precise = (value - p.min) / (p.max-p.min) * (lib.expd.numCandidates-1);

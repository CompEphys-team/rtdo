...
void GAFitter::cl_record_validation(QFile &base)
{
    const RunData &rd = session.runData();
    ...
    for ( int iT = 0; iT < maxDuration; iT++ ) {
        for ( const std::vector<double> &trace : V ) {
...
{
    std::vector<std::vector<double>> V(nStims, std::vector<double>(maxDuration));

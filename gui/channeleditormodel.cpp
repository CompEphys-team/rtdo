...
QVariant ChannelEditorModel::data(const QModelIndex & index, int role) const {
    ...
    if ( role == Qt::EditRole ) {
        ...
        case ReadOffsetSource:
            if ( (it = std::find(channels.begin(), channels.end(), c->read_offset_src)) == channels.end() )
                return QVariant(-1);
            else
                return QVariant((int)(it - channels.begin()));
...
bool ChannelEditorModel::insertRow(int row, const QModelIndex &parent) {
    ...
    channels.insert(channels.begin() + row, 1, chan);
    endInsertRows();
    emit channelsUpdated();
    return true;
}
...
bool ChannelEditorModel::removeRow(int row, const QModelIndex &parent) {
    ...
    for ( std::vector<daq_channel *>::iterator it = channels.begin(); it != channels.end(); ++it ) {
